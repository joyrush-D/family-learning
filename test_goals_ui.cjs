...
runpy.run_path('demo.py',run_name='__main__')`],{cwd:__dirname,env,stdio:['ignore','pipe','pipe']});server.stdout.resume();server.stderr.resume();
 ...
 for(const width of [360,1440]){
  ...
  const reopenWordHistory=async()=>{await p.reload();await p.locator('nav [data-page="more"]').click();await p.locator('.more-links [data-page="goals"]').click();const choice=p.locator(`[data-goal-select="${wordSnapshot.id}"]`);if(await choice.count())await choice.click();await p.locator('[data-word-check] > summary').click();await history.waitFor()};
  ...
  await p.reload();await p.locator('nav [data-page="more"]').click();await p.locator('.more-links [data-page="goals"]').click();const memorySelect=p.locator(`[data-goal-select="${savedGoal.id}"]`);if(await memorySelect.count())await memorySelect.click();
  ...
  await p.reload();await p.locator('nav [data-page="more"]').click();await p.locator('.more-links [data-page="goals"]').click();await p.locator('[data-goal-child-select="child-2"]').click();const sourceGoalSelect=p.locator(`[data-goal-select="${autoGoal.id}"]`);if(await sourceGoalSelect.count())await sourceGoalSelect.click();await p.locator('#goal-school-requirement summary').click();
