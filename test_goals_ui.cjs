...
runpy.run_path('demo.py',run_name='__main__')`],{cwd:__dirname,env,stdio:['ignore','pipe','pipe']});server.stdout.resume();server.stderr.resume();
 ...
 for(const width of [360,1440]){
  ...
  await reopenWordHistory();if(!(await profile.evaluate(d=>d.open)))await profile.locator('summary').click();
  const correctedProfile=(await(await p.request.get(url+'api/goals')).json()).goals.find(g=>g.id===secondProfile);assert.equal(correctedProfile.records.filter(r=>r.id===Number(profileOriginalID)).length,1);assert.equal(correctedProfile.word_history.words.find(w=>w.word==='torch').directions.meaning_spelling.status,'提示后答对');assert.deepEqual(correctedProfile.current_plan,childProfileBefore.current_plan);assert.deepEqual(correctedProfile.hypotheses_detail[0].corrected,[originalProfileRecord],'the corrected citation is named on that judgment');assert.match(await profile.locator('[data-profile-judgment]').filter({hasText:'虚构画像范围 '+width}).locator('[data-profile-corrected]').innerText(),/它引用的原记录在确认后已更正[\s\S]*失去原依据，待重新评估/);checks++;
  ...
  const reviewed=p.locator('[data-goal-reviewed-evidence]');await reviewed.locator('summary').click();assert.match(await reviewed.innerText(),/以下是当时的引用/);const reviewedQuotes=await reviewed.locator('blockquote').allTextContents();assert.ok(reviewedQuotes.some(text=>text.startsWith('虚构作业反馈')));assert.ok(reviewedQuotes.some(text=>text.startsWith('虚构老师要求')),'the approved teacher requirement stays beside the feedback');
