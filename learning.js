...
function learningJourneysHTML() {
 ...
 return `<section class="card learning-journeys" aria-labelledby="learning-journeys-title"><div class="learning-journeys-heading"><div><p class="small muted">从一次具体困难，跟到下一次尝试</p><h2 id="learning-journeys-title">订正之后，后来怎么样了？</h2></div><div class="learning-count"><strong>${cases.length}</strong><span>条学习线索</span></div></div><p class="muted">沿着你实际关联的记录看变化。帮助情况、题目关系和当时表现都能补充、更正。</p>${missing?`<p class="learning-next">${missing} 条线索已有订正，尚无复测记录。可以在下一次实际尝试后补记。</p>`:''}<div class="learning-case-list">${cases.map(g=>{
  ...
  const root=g.root,summary=retests.length?`有 ${retests.length} 条复测记录，请展开核对当时帮助和材料关系。`:correction?`有 ${correction} 条订正记录，尚无复测记录。`:'还没有复测记录，可以先记下当时的尝试与困难。';
  return `<article class="learning-case" data-learning-case="${esc(root.id)}"><div class="learning-case-top"><span class="chip">${esc(root.child)}${root.subject?' · '+esc(root.subject):''}</span><span class="small muted">${esc(root.day||'日期待核对')}</span></div><h3>${esc(root.title||'标题未记录')}</h3><div class="learning-route" aria-label="这条线索已记录的步骤"><span>起始记录</span><span aria-hidden="true">→</span><span>订正 ${correction}</span><span aria-hidden="true">→</span><span>复测 ${retests.length}</span>${observations?`<span>观察 ${observations}</span>`:''}</div><p class="learning-case-summary">${esc(summary)}</p>${g.issues.map(s=>`<p class="learning-limit">${esc(s)}</p>`).join('')}<details class="learning-compare"><summary>展开对照 · ${g.items.length} 条记录</summary><p class="small muted">每条跟进针对下方注明的直接关联记录。日期差仅来自记录日期，不代表实际教学或练习间隔。</p><div class="learning-steps">${g.items.map((r,i)=>rowHTML(r,g,i)).join('')}</div></details>${canEdit(root)?`<div class="learning-actions"><button type="button" data-guided-new="${esc(root.id)}">一起试这题</button><button type="button" data-followup="${esc(root.id)}" data-followup-kind="订正">记一次订正</button><button type="button" data-followup="${esc(root.id)}" data-followup-kind="复测">记一次复测</button><button type="button" data-followup="${esc(root.id)}" data-followup-kind="补充观察">补充观察</button></div>`:''}</article>`;
 }).join('')||empty('还没有可跟进的个人学习记录。先保存一次尝试或一张试卷，再关联订正和之后的复测。')}</div><p class="small muted learning-footnote">这些是已填写的过程记录，不自动核实作答过程，也不据分数、记录数量或一次尝试判定掌握。</p></section>`;
}
