...
function learningCases(records, selectedChild='') {
 ...
 for(const r of rows){
  if(!['成绩','学习进展','课程进度'].includes(r.category)&&!r.related_record_id&&!r.followup_kind)continue;
...
function learningIsPractice(g){return g.items.some(r=>r.category!=='课程进度'&&(r.category==='成绩'||['订正','复测','独立复测'].includes(r.followup_kind)))}
...
function learningJourneysHTML() {
 const cases=learningCases(data.records||[],child),missing=cases.filter(g=>g.items.some(r=>r.category!=='课程进度'&&r.followup_kind==='订正')&&!g.items.some(r=>r.category!=='课程进度'&&['复测','独立复测'].includes(r.followup_kind))).length;
 ...
 const rowHTML=(r,g,index)=>{
  ...
  const stage=r.category==='课程进度'?'课程进度 · 非个人表现':index===0?(r.followup_kind&&!r.related_record_id?r.followup_kind+' · 未关联':g.issues.length?'起点待核对':'原始记录'):(r.followup_kind||'跟进类型未记录');
 ...
 return `<section class="card learning-journeys" aria-labelledby="learning-journeys-title"><div class="learning-journeys-heading"><div><h2 id="learning-journeys-title">从已有记录继续</h2></div><div class="learning-count"><strong>${cases.length}</strong><span>条学习线索</span></div></div>${missing?`<p class="learning-next">${missing} 条线索已有订正，尚无复测记录。可以在下一次实际尝试后补记。</p>`:''}<div class="learning-case-list">${cases.map(g=>{
  const correction=g.items.filter(r=>r.category!=='课程进度'&&r.followup_kind==='订正').length,retests=g.items.filter(r=>r.category!=='课程进度'&&['复测','独立复测'].includes(r.followup_kind)),observations=g.items.filter(r=>r.followup_kind==='补充观察').length;
