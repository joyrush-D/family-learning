...
(async()=>{let proc,browser;try{
 ...
 for(const width of [360,1440]){
  ...
  await page.reload();await page.locator('.today-dashboard').waitFor();await page.locator('nav [data-page="more"]').click();await page.locator('#content [data-page="agent"]').click();await page.locator('#content h1').waitFor();
  ...
  const planned=page.locator('[data-agent-item]').filter({hasText:'虚构时间管理小动作 '+width});await planned.scrollIntoViewIfNeeded();await proof(page,'planned-care-'+width);assert.match(await planned.innerText(),/具体动作|目标：|为什么现在：|25 分钟/);const plannedID=await planned.getAttribute('data-agent-item');await planned.locator('[data-agent-accept]').click();await page.locator('#agentDialog').waitFor();assert.equal(await page.locator('#agentForm [name="review_on"]').inputValue(),current.today);await page.locator('#agentForm [name="title"]').fill('虚构已核对的时间安排 '+width);await page.locator('#agentForm [name="body"]').fill('19:00 开始，完成一轮后记录实际分钟数');await page.locator('#agentForm [name="estimated_minutes"]').fill('30');await page.route('**/api/agent/action',async r=>{await r.fetch();await r.abort('connectionreset')},{times:1});await page.locator('#agentForm [type="submit"]').click();await until(()=>page.locator('#agentError').textContent().then(t=>Boolean(t)),'planned care failure visible');assert.equal(await page.locator('#agentForm [name="body"]').inputValue(),'19:00 开始，完成一轮后记录实际分钟数');await page.locator('#agentForm [type="submit"]').click();await until(()=>page.locator('#agentDialog').isVisible().then(v=>!v),'planned care accepted');await page.unroute('**/api/agent/action');let plannedState=await state();assert.equal(plannedState.agent.items.filter(x=>x.id===plannedID).length,1);const plannedTask=plannedState.tasks.find(t=>t.title==='虚构已核对的时间安排 '+width);assert.ok(plannedTask);await page.locator('details.card').filter({hasText:'虚构已核对的时间安排 '+width}).locator('summary').first().click();const acceptedPlanned=page.locator('[data-task="'+plannedTask.id+'"]').locator('xpath=ancestor::article[@data-agent-item]');assert.ok(await acceptedPlanned.count());await acceptedPlanned.locator('[data-agent-defer]').click();await page.locator('#agentDeferDialog').waitFor();const deferredDate=new Date(Date.parse(current.today+'T12:00:00Z')+7*86400000).toISOString().slice(0,10);await page.locator('#agentDeferForm [name="review_on"]').fill(deferredDate);
  const competingDate=new Date(Date.parse(current.today+'T12:00:00Z')+5*86400000).toISOString().slice(0,10);
  const competing=await fetch(url+'api/agent/action',{method:'POST',headers:{'Content-Type':'application/json','X-Family-Token':token},body:JSON.stringify({id:plannedID,action:'defer',review_on:competingDate,expected_updated:plannedState.agent.items.find(x=>x.id===plannedID).updated})});assert.equal(competing.status,200);
  await page.locator('#agentDeferForm [type="submit"]').click();await page.locator('[data-agent-refresh-defer]:visible').waitFor();assert.equal(await page.locator('#agentDeferForm [name="review_on"]').inputValue(),deferredDate,'real CAS conflict preserves the proposed date');
  await page.locator('[data-agent-refresh-defer]').click();await until(()=>page.locator('#agentDeferCurrent').innerText().then(t=>t.includes(competingDate)),'latest parent decision visible before overwrite');assert.equal(await page.locator('#agentDeferForm [name="review_on"]').inputValue(),deferredDate);await fits(page);
  await page.locator('#agentDeferForm [type="submit"]').click();await until(()=>page.locator('#agentDeferDialog').isVisible().then(v=>!v),'planned care deferred');const deferredState=await state();assert.equal(deferredState.agent.items.find(x=>x.id===plannedID).plan.approved.review_on,deferredDate);await page.reload();await page.locator('.today-dashboard').waitFor();await page.locator('nav [data-page="more"]').click();await page.locator('#content [data-page="agent"]').click();await page.locator('#content h1').waitFor();await page.locator('details.card').filter({hasText:'虚构已核对的时间安排 '+width}).locator('summary').first().click();await until(async()=>page.locator('[data-agent-item="'+plannedID+'"]').innerText().then(t=>t.includes(deferredDate)),'deferred date visible');await page.locator('[data-agent-item="'+plannedID+'"]').scrollIntoViewIfNeeded();await proof(page,'planned-care-confirm-'+width);
  assert.deepEqual(pageErrors,[]);await page.close();
 }
 const p=await browser.newPage();await p.goto(url);await p.locator('.today-dashboard').waitFor();await p.locator('nav [data-page="more"]').click();await p.locator('#content [data-page="agent"]').click();await p.locator('#content h1').waitFor();const recordsBeforeDismiss=(await state()).records.length;await p.locator('[data-agent-dismiss]').first().click();await until(async()=>!(await state()).agent.items.some(x=>x.kind==='care'&&x.state==='pending'),'dismiss saved');assert.equal((await state()).records.length,recordsBeforeDismiss,'acknowledgement does not fabricate child feedback');await p.close();
 console.log(JSON.stringify({passed:true,widths:[360,1440],realAPI:true,syntheticOnly:true,formRetry:true,childBinding:true,escapedEvidence:true,acknowledgementNotFeedback:true,schoolSourceEntry:true,actualDayNotGuessed:true,taskFeedbackPreserved:true,sameSourceSameCase:true,observationSameChain:true,stableRecordRetry:true,closedUnknownReceiptRecovered:true,recordDoesNotChangeTask:true,emptySourceNoDefaultChild:true,ordinarySchoolEventNoForcedRetest:true,plannedCareLostReceipt:true,plannedCareConcurrentDeferral:true}));
