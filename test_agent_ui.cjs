...
(async()=>{let proc,browser;try{
 ...
 for(const width of [360,1440]){
  ...
  const changeToken=changeBefore.token;const conflictReply=await page.request.post(url+'api/task',{headers:{'X-Family-Token':changeToken},data:{id:changeTask.id,status:'待跟进',note:'虚构另一位家长补充了帮助情况。'}});assert.equal(conflictReply.status(),200);
