...
(async()=>{
 ...
 try{
  ...
  for(const width of [360,1440]){
   ...
   await parent.goto(server.url);await parent.locator('[data-study-child="'+who+'"]').click();await parent.locator('[data-study-ready]').waitFor();await parentRow.getByRole('button',{name:'核对孩子的结果'}).click();const confirm=parent.locator('[data-study-form="finish"]');assert.equal(await confirm.locator('[value="完成"]').isChecked(),true);await confirm.getByRole('button',{name:'核对并保存'}).click();
