...
(() => {
 ...
 function itemHTML(item){
  ...
  return `<article class="study-item ${running?'study-running':''}" data-study-item="${esc(item.id)}"><div class="study-item-top"><div><span class="study-meta">${esc(item.subject||'作业')}${item.task_id?' · 已有待办':''} · 预计 ${item.planned_minutes===null?'待填写':esc(item.planned_minutes)+' 分钟'}</span><h3>${esc(item.title)}</h3></div><span class="study-timer" ${running?`data-study-clock="${esc(item.id)}"`:''}>${ended?esc(minutes(item.actual_minutes)):timerText(item)}</span></div>${source}${window.FamilyHomework?.reportHTML(item.report,id=>new URL('upload/'+id,location.href).href)||''}${item.report?.actor==='child'&&!item.report.confirmed_at?`<button data-study-action="confirm_report" data-id="${esc(item.id)}">已核对这项作业要求</button>`:''}${ended?`<p>${esc(item.result)}${item.assistance?' · '+esc(item.assistance):''}${item.result_actor==='child'?' · 孩子自述，待家长核对':''}</p>`:running?'<p class="study-meta">正在计时 · 关闭网页仍会继续，休息时请暂停</p>':item.status==='paused'?'<p class="study-meta">已暂停</p>':''}${dismissed?`<p class="study-meta">原事项：${item.source_task_status==='不适用'?'无需处理':'不参加'} · 已移出今晚安排，已记用时保留</p>`:''}${item.time_needs_review?'<p class="study-warning">这段计时可能包含离开或跨日时间，请核对实际分钟。</p>':''}${item.note?`<p class="study-note">${esc(item.note)}</p>`:''}<div class="study-actions study-primary-actions">${mainActions}</div><div class="study-actions study-secondary-actions">${secondaryActions}</div></article>`;
 }
 function editorHTML(){
  if(!editor)return '';
 ...
 function paint(preserve=true){
  ...
  el.innerHTML=`<section class="study-view" ${snapshot?'data-study-ready="true"':''}><header class="study-heading"><h1>作业执行</h1><div class="study-picker"><label><span class="sr-only">孩子</span><select data-study-child aria-label="孩子">${ctx.children.map(c=>`<option value="${esc(c.id)}"${chosen(ctx.child_id,c.id)}>${esc(c.name)}</option>`).join('')}</select></label><label><span class="sr-only">查看日期</span><input type="date" data-study-date aria-label="查看日期" value="${esc(ctx.day)}" max="${today()}"></label></div></header><div id="studyStatus" class="study-status" role="status" aria-live="polite">${esc(message)}${pending?'<button data-study-retry>核对并重试这次保存</button>':''}</div>${snapshot?`${activeHTML()}${budgetHTML()}${snapshot.day.closed_at?`<p class="study-closed">${esc(clockText(snapshot.day.closed_at))} 已收尾 · 仍可补充结果</p>`:''}<div class="study-list">${itemsHTML||'<p class="study-empty">这里记录开始、暂停、用时和结果。先从日历选择课内作业，临时口头作业也可以补记。</p>'}${editor&&!find(editor.id)?editorHTML():''}</div><p><button data-homework-capture>用登记本 / 语音报功课</button></p><details class="study-add" ><summary>＋ 补记口头作业</summary><form data-study-form="new"><label>从收集箱选择作业 · 可选<select name="task_id"><option value="">自己填写</option>${snapshot.available_tasks.map(t=>`<option value="${esc(t.id)}">${esc(t.title)}</option>`).join('')}</select></label>${fields()}<button class="primary" type="submit">加入今晚</button><button type="button" data-study-discard-new>清空草稿</button><p class="study-meta">未提交内容暂存在当前网页，切页可继续；确认加入后才保存到家庭记录。</p></form></details><p><button data-page="calendar">回到日历选作业</button></p>${settingsHTML()}${weekHTML()}<div class="study-footer"><button data-study-close ${snapshot.day.day!==today()||snapshot.day.closed_at||!snapshot.items.length||snapshot.active_item||snapshot.items.some(i=>i.status==='running')?'disabled':''}>今晚收尾</button><button data-study-refresh>刷新已保存状态</button><span class="study-meta">${snapshot.summary.unfinished_count} 项尚需跟进</span></div>`:'<p class="study-empty">正在读取放学后的安排…</p>'}</section>`;
