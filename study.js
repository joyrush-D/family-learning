...
(() => {
 ...
 function budgetHTML(){
  ...
  return `<section class="study-budget" aria-label="今晚时间预算"><div><span>剩余可用 · ${esc(clockText(readAt))} 更新</span><strong>${s.remaining_available_minutes===null||s.remaining_available_minutes===undefined?'时间待核对':`${esc(s.remaining_available_minutes)} 分钟`}</strong></div><div><span>功课原预估余额</span><strong>${esc(s.remaining_minutes)} 分钟${s.unknown_remaining_count?' + 待估 / 重估':''}</strong></div><p class="study-meta">整晚 ${esc(range)} · 预计功课 ${esc(s.planned_minutes)} 分钟${s.unknown_estimates?'，尚未估全':''}${s.available_minutes===null?'':` · 可用 ${esc(s.available_minutes)} 分钟`}</p>${d.bed_time?`<p>准备睡觉 ${esc(d.bed_time)}</p>`:''}${s.warning?`<p class="study-warning">${esc(s.warning)}</p>`:''}${s.source_gap?`<details><summary>安排信息待核对</summary><p>${esc(s.source_gap)}</p></details>`:''}</section>`;
 }
 ...
 function itemOrder(item){return ['不参加','不适用'].includes(item.source_task_status)?2:item.status==='running'?-1:item.result==='完成'?1:0}
 function itemHTML(item){
  ...
  return `<article class="study-item ${running?'study-running':''}" data-study-item="${esc(item.id)}"><div class="study-item-top"><div><span class="study-meta">${esc(item.subject||'作业')}${item.task_id?' · 已有待办':''} · 预计 ${item.planned_minutes===null?'待填写':esc(item.planned_minutes)+' 分钟'}</span><h3>${esc(item.title)}</h3></div><span class="study-timer" ${running?`data-study-clock="${esc(item.id)}"`:''}>${ended?esc(minutes(item.actual_minutes)):timerText(item)}</span></div>${source}${ended?`<p>${esc(item.result)}${item.assistance?' · '+esc(item.assistance):''}${item.result_actor==='child'?' · 孩子自述，待家长核对':''}</p>`:running?'<p class="study-meta">正在计时 · 关闭网页仍会继续，休息时请暂停</p>':item.status==='paused'?'<p class="study-meta">已暂停</p>':''}${dismissed?`<p class="study-meta">原事项：${item.source_task_status==='不适用'?'无需处理':'不参加'} · 已移出今晚安排，已记用时保留</p>`:''}${item.time_needs_review?'<p class="study-warning">这段计时可能包含离开或跨日时间，请核对实际分钟。</p>':''}${item.note?`<p class="study-note">${esc(item.note)}</p>`:''}<div class="study-actions">${dismissed?`<button data-study-task="${esc(item.task_id)}">查看决定 / 恢复跟进</button><button data-study-editor="manual" data-id="${esc(item.id)}">核对已记用时</button>`:!ended?`${running?`<button data-study-action="pause" data-id="${esc(item.id)}">暂停</button>`:`<button class="primary" data-study-action="start" data-id="${esc(item.id)}" ${old||active?'disabled':''}>${item.status==='paused'?'继续':'开始'}</button>`}<button data-study-editor="finish" data-id="${esc(item.id)}">结束这项</button><button data-study-editor="item" data-id="${esc(item.id)}">改预计</button><button data-study-editor="manual" data-id="${esc(item.id)}">补记用时</button>`:`<button data-study-editor="finish" data-id="${esc(item.id)}">${item.result_actor==='child'?'核对孩子的结果':'更正结果'}</button>${item.result!=='完成'?`<button data-study-action="start" data-id="${esc(item.id)}" ${old||active?'disabled':''}>继续这项</button>`:''}<button data-study-editor="manual" data-id="${esc(item.id)}">更正用时</button>${item.record_id?`<button data-study-record="${esc(item.record_id)}">学习记录 / 后续尝试</button>`:''}`}${item.task_id&&!dismissed?`<button data-study-task="${esc(item.task_id)}">查看原事项</button>`:''}</div></article>`;
 }
 function editorHTML(){
  if(!editor)return '';
  const item=find(editor.id),v=editor.values||item||{},type=editor.type;
 ...
 function paint(preserve=false){
  ...
  const oldForms=preserve?[...el.querySelectorAll('form')].map(f=>({kind:f.dataset.studyForm,values:[...new FormData(f)],open:f.closest('details')?.open})):[];
  el.innerHTML=`<section class="study-view" ${snapshot?'data-study-ready="true"':''}><header class="study-heading"><h1>今日作业</h1><div class="study-picker"><label><span class="sr-only">孩子</span><select data-study-child aria-label="孩子">${ctx.children.map(c=>`<option value="${esc(c.id)}"${chosen(ctx.child_id,c.id)}>${esc(c.name)}</option>`).join('')}</select></label><label><span class="sr-only">查看日期</span><input type="date" data-study-date aria-label="查看日期" value="${esc(ctx.day)}" max="${today()}"></label></div></header><div id="studyStatus" class="study-status" role="status" aria-live="polite">${esc(message)}${pending?'<button data-study-retry>核对并重试这次保存</button>':''}</div>${snapshot?`${activeHTML()}${budgetHTML()}${snapshot.day.closed_at?`<p class="study-closed">${esc(clockText(snapshot.day.closed_at))} 已收尾 · 仍可补充结果</p>`:''}<div class="study-list">${[...snapshot.items].sort((a,b)=>itemOrder(a)-itemOrder(b)).map(itemHTML).join('')||'<p class="study-empty">说说今天要做的作业。一项一项添加，也可以用手机键盘语音输入。</p>'}</div>${editorHTML()}<details class="study-add" ${!snapshot.items.length?'open':''}><summary>＋ 添加一项作业</summary><form data-study-form="new"><label>从已有待办选择 · 可选<select name="task_id"><option value="">自己填写</option>${snapshot.available_tasks.map(t=>`<option value="${esc(t.id)}">${esc(t.title)}</option>`).join('')}</select></label>${fields()}<button class="primary" type="submit">加入今晚</button></form></details>${settingsHTML()}${weekHTML()}<div class="study-footer"><button data-study-close ${snapshot.day.day!==today()||snapshot.day.closed_at||!snapshot.items.length||snapshot.active_item||snapshot.items.some(i=>i.status==='running')?'disabled':''}>今晚收尾</button><button data-study-refresh>刷新已保存状态</button><span class="study-meta">${snapshot.summary.finished_count} 项完成 · ${snapshot.summary.unfinished_count} 项尚需跟进</span></div>`:'<p class="study-empty">正在读取放学后的安排…</p>'}</section>`;
  for(const old of oldForms){const form=el.querySelector(`form[data-study-form="${old.kind}"]`);if(!form)continue;if(old.kind!=='day'&&old.kind!=='new'&&!editor)continue;for(const [name,value] of old.values){const field=form.elements[name];if(field instanceof RadioNodeList){for(const radio of field)radio.checked=radio.value===value}else if(field)field.value=value}if(old.open&&form.closest('details'))form.closest('details').open=true}
 ...
 function consumeTask(){
  const taskID=ctx?.task_id;if(!taskID||pending)return;
  ctx={...ctx,task_id:''};ctx.onTaskSelected?.();
  const existing=snapshot.items.find(item=>String(item.task_id)===String(taskID));
  if(existing){const target=[...root()?.querySelectorAll('[data-study-item]')||[]].find(item=>item.dataset.studyItem===String(existing.id));if(target){target.tabIndex=-1;target.scrollIntoView({block:'center'});target.focus({preventScroll:true})}return}
  if(!snapshot.available_tasks.some(task=>String(task.id)===String(taskID)))return;
  ...
  form.elements.task_id.value=String(taskID);setSource(form);form.scrollIntoView({block:'center'});
 }
 ...
 async function save(){
  ...
  try{
   ...
   pending=null;editor=null;message='已保存';if(root()){for(const f of root().querySelectorAll('form'))f.reset()}
   if(ctx===active){snapshot=result;readAt=Date.now();paint()}
 ...
 function click(e){
  ...
  if(button.hasAttribute('data-study-refresh')){message='';read()}
  if(button.hasAttribute('data-study-cancel')){editor=null;paint()}
  if(button.dataset.studyEditor){const item=find(button.dataset.id);if(!item)return;editor={type:button.dataset.studyEditor,id:item.id};paint();root()?.querySelector('.study-editor')?.scrollIntoView({block:'nearest'});root()?.querySelector('.study-editor input:not([readonly])')?.focus()}
  ...
  if(button.dataset.studyDay){ctx={...ctx,day:button.dataset.studyDay};snapshot=null;editor=null;message='';paint();read()}
 ...
 function submit(e){
  ...
  const type=form.dataset.studyForm,v=Object.fromEntries(new FormData(form)),item=find(editor?.id),number=value=>value===''?null:Number(value);
  if(type==='day')newRequest('/api/study/day',{...v,version:snapshot.day.version});
  if(type==='new'||type==='item'){const body={subject:v.subject,planned_minutes:number(v.planned_minutes),version:item&&type==='item'?item.version:0};if(type==='item'){body.id=item.id;if(!item.task_id)body.title=v.title}else if(v.task_id)body.task_id=v.task_id;else body.title=v.title;newRequest('/api/study/item',body)}
  if(type==='finish'||type==='manual'){const body={id:item.id,version:item.version,action:type};if(type==='finish'){body.result=v.result;body.assistance=v.assistance;body.note=v.note}if(v.actual_minutes!=='')body.actual_minutes=Number(v.actual_minutes);newRequest('/api/study/action',body)}
 }
