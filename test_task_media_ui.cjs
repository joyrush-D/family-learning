...
(async()=>{let browser,server;const checks=[];try{
 ...
 for(const width of [360,1440]){
  ...
  assert.equal(reviewBodies.length,3);assert.deepEqual(reviewBodies[0],{record_id:videoRecord.id,upload_id:uploadID,expected_token:tokenA,action:'confirm',selected:[1]});assert.deepEqual(reviewBodies[1],reviewBodies[0]);assert.deepEqual(reviewBodies[2],reviewBodies[0]);
  ...
  await eventually(async()=>/不是当前版本/.test(await panel.innerText()),'stale token refused');assert.equal(reviewBodies.length,4);assert.equal(reviewBodies[3].expected_token,tokenA);assert.deepEqual(reviewBodies[3].selected,[0]);
  ...
  assert.equal(await panel.locator('[data-video-obs]:checked').count(),0);assert.equal(await revoke.count(),0);assert.equal(await panel.locator('[data-video-review="confirmed"]').count(),0);assert.equal(reviewBodies.length,4);
  ...
  assert.deepEqual(reviewBodies[4],{record_id:videoRecord.id,upload_id:uploadID,expected_token:tokenB,action:'revoke',selected:[]});assert.equal(await panel.locator('[data-video-review="confirmed"]').count(),0);assert.match(await panel.innerText(),/尚未核对/);assert.match(await panel.innerText(),/2026-09-23 09:10/);
  ...
  await p.unroute(reviewRoute);assert.equal(reviewBodies.length,6);
