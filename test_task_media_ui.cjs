...
(async()=>{let browser,server;const checks=[];try{
 ...
 for(const width of [360,1440]){
  ...
  let leaveAction='accept',leavePrompts=0;
  p.on('dialog',async d=>{if(d.type()==='beforeunload'){await d.accept();return}assert.equal(d.type(),'confirm');leavePrompts++;await d[leaveAction]()});
  const save=p.locator('#saveTaskFeedback');await open();
  await p.locator('[data-close="taskDialog"]').click();assert.equal(leavePrompts,0);await open();
  await p.locator('#taskForm [name=note]').fill('虚构：订正到第三题，稍后继续');leaveAction='dismiss';
  await p.locator('[data-close="taskDialog"]').click();assert.equal(leavePrompts,1,'unsaved note must prompt before close');
  assert(await p.locator('#taskDialog').evaluate(x=>x.open));
  await p.keyboard.press('Escape');assert.equal(leavePrompts,2);assert(await p.locator('#taskDialog').evaluate(x=>x.open));
  assert.equal(await p.locator('#taskForm [name=note]').inputValue(),'虚构：订正到第三题，稍后继续');
  const warnsOnUnload=()=>p.evaluate(()=>{const e=new Event('beforeunload',{cancelable:true});window.dispatchEvent(e);return e.defaultPrevented});
  assert(await warnsOnUnload());
  await p.route('**/api/task/feedback',r=>r.fulfill({status:422,json:{error:'虚构保存失败'}}));
  await save.click();await eventually(async()=>/输入仍保留/.test(await p.locator('#taskError').innerText()),'known failure retains input');
  await p.locator('[data-close="taskDialog"]').click();assert.equal(leavePrompts,3);assert(await p.locator('#taskDialog').evaluate(x=>x.open));
  await p.unroute('**/api/task/feedback');
  if(process.env.TASK_MEDIA_UI_PROOF_DIR){await fs.mkdir(process.env.TASK_MEDIA_UI_PROOF_DIR,{recursive:true});await p.screenshot({path:path.join(process.env.TASK_MEDIA_UI_PROOF_DIR,'unsaved-feedback-'+width+'.png')})}
  ...
  leaveAction='dismiss';prompts=leavePrompts;await p.keyboard.press('Escape');assert.equal(leavePrompts,prompts+1);
  assert.equal(await p.locator('#taskTranscript').inputValue(),'虚构核对：第三个词听不清');leaveAction='accept';
  ...
  assert.equal(await p.locator('#taskTranscript').isDisabled(),true);
  await p.keyboard.press('Escape');assert(await p.locator('#taskDialog').evaluate(x=>x.open));
  ...
  await p.locator('[data-task-feedback-edit]').first().click();prompts=leavePrompts;await p.keyboard.press('Escape');assert.equal(leavePrompts,prompts,'unchanged existing feedback is clean');await open();
  ...
  await p.locator('#taskForm [name=status]').selectOption('待跟进');assert(!await warnsOnUnload());
