 ...
 try{
  ...
  for(const width of [360,1440]){
   ...
   try{
    ...
    await post('api/task/focus',focusBody);await p.reload();await eventually(async()=>await p.locator('[data-study-task-add="'+sourceTask.id+'"]').count()===1,'school task opens study');for(const details of await p.locator('.today-backlog').all())if(!await details.evaluate(el=>el.open))await details.locator(':scope > summary').click();await p.locator('[data-study-task-add="'+sourceTask.id+'"]').click();await p.locator('[data-study-ready]').waitFor();await eventually(async()=>await add.locator('[name="task_id"]').inputValue()===sourceTask.id,'task preselected from school card');assert.equal(await add.locator('[name="title"]').inputValue(),title,'task title prefilled');assert.equal(await add.locator('[name="title"]').getAttribute('readonly'),'','task title stays source-owned');assert.equal(await p.locator('.study-add').evaluate(el=>el.open),true,'task card expands add form');
    await add.locator('[name="subject"]').fill('数学');await add.locator('[name="planned_minutes"]').fill('25');
