...
from family_wechat_media import MediaError, validate_png
...
async def capture_once(config, data, local):
    ...
    plan = client.request('/api/agent/collector', deadline=deadline)
    ...
    try:
        with tempfile.TemporaryDirectory(prefix='.qq-capture-', dir=data) as temporary:
            ...
            current = client.request('/api/agent/collector', deadline=deadline)
