...
已有远端主库不能直接改启动路径就视为已迁移。应先停写并生成一致备份，在隔离目录恢复核对记录、附件、来源游标、孩子会话与打印状态，确认访问与回退方案，再切换唯一写入点；不启动旧镜像作为第二个主库。完整切换和真实设备验收尚未完成。
...
2026-09-09已在同一个隔离虚构家庭中，通过网页设置在本地Qwen与豆包Responses之间切换，分别完成日历查询、教学草稿、独立进程整理通知及核对后生成待办。两种模型实际请求均成功，原档案与日历保留，重启不重复调用。此验证覆盖接口和状态兼容；本地模型的部分教学步骤仍偏通用，教学质量尚需改善，不能把格式通过当作效果合格。试用家庭的生产模型未据此自动切换。
