...
r70源码复核时未执行切换；r71已对当时验收main完成两次备份/隔离恢复核验并更新家庭实例，原主库和公网入口保留。r74再次按本节门槛完成升级前/停写后备份、42表隔离恢复、公开文件一致性、来源游标、本机接口和认证公网入口核对，并恢复原网页、Agent、采集、打印及隧道服务。手机/桌面宽度的软件检查与实际家长入口分别留证。后续增强不阻止已通过当前门槛的独立批次上线；真实手机、微信内访问、打印出纸和持续教学效果仍需分别验收。现有微信采集宿主需要保留进程时，可在短暂停写窗口暂停并恢复原进程及其子进程，不借升级重启客户端或触发重新授权。
