...
1. **备份与演练。** 用当前运行代码的 `family_backup.py --root 应用目录 create 私有备份.zip` 生成完整备份，访问/模型配置、服务文件、消息工具配置和旧代码另存私有副本；这些配置不全在数据归档内。用候选版本的 `family_backup.py restore 私有备份.zip 全新空目录` 恢复，检查归档校验和、SQLite完整性、业务数量与关联、原件、日历UID及来源游标。恢复工具会停用Agent、清除会话/邀请、暂缓打印及计时，故恢复后数据库字节不应要求等同原包；原包哈希须先通过。配置文件与核验材料不公开。
...
本轮画像源码复核未执行上述家庭切换与恢复演练；后续考试/老师评测闭环稳定后，按同一冻结提交逐项留证再发布到家庭实例。真实手机、微信内访问与打印出纸仍需分别验收。
