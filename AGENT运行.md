...
不新增服务，复用每分钟的Agent进程；默认关闭，手动上传和通知核对继续独立工作。当前适配 `wechat-cli 1.6.19` 的一个已核验构建，SHA-256固定在 `family_media.py`。不同构建在核对禁止取钥的行为前会被拒绝。依赖普通Python 3.11、macOS CommonCrypto和部署者已有的FFmpeg；不使用Codex运行环境，也不打包这些外部工具。
...
每轮最多处理一条已保存、仍授权的图片；消息原文、未读状态、作业及奖励保持原值。只读取CLI对该条消息返回的当前账号附件路径，拒绝符号链接和目录外文件。原图与缩略图候选按本构建的资源变体及大小匹配；当前仅转换已验证V2/WXGF布局中的HEVC首帧，不承诺所有版本、照片、动画或文件类型。PNG显示在“原通知与原件”，文件名标明首帧。

CLI固定使用已存在的 `/usr/bin/false` 作为取钥程序，并停用元数据自动刷新；不传入其他取钥或密钥环境变量。它仍会读取本机数据库并写自己的解码缓存，不能称为零写入。CLI与FFmpeg分别限制15秒，输出、图片大小和像素均有上限。缺权限、密钥或支持格式时保留缺口，不自动申请权限或提取新密钥；同一消息至少隔5分钟重试，总计最多3次，包括中断的轮次。用网页手动补图即可继续，不需要等自动取图恢复。
