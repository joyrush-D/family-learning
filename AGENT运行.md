...
- 消息所在电脑：运行 `family_collect.py --config private/collector.json --interval 300`。微信复用这台机器上实际可用、已授权的 CLI；QQ 的现有 CLI 接口和已授权 Cua 保底路线见 V1 的 S02，Cua 尚未完成独立采集适配，不能写入 qq_cli 冒充原生消息接口。个人登录态不复制到应用主机。
