...
首版每个订阅最多1000条原始安排（重复系列计一条），每个系列最多8个时段事件，保留历史与取消。每日、周末、指定星期/月日沿保存规则生成；没有所选月日时跳过，指定日以家庭时区计算。原系列与额外时段使用稳定标识，修改仍影响整条计划，逐次例外尚未实现。超限或来源读取失败返回错误，不截断或用空日历覆盖。课表节次不导出，未知钟点作为透明的时间待定日期，原始消息与备注不进入订阅。协议和认证接口检查通过不等于真实 iPhone 已同步；首次认证与后续刷新需家庭实测。
