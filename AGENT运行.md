...
同一任务总尝试上限已实现。其余成本要求是按明确任务选择轻模型或默认模型、限制每项上下文与输出、给总尝试设限，并记录实际调用用量与升级原因；规则采集、去重与到期判断继续不调用模型。当前任务路由和产品内用量记录尚未实现，不能将每轮三次当作整日或整项预算。模型返回的用量缺失时保留未知，不用输出上限冒充实际token消耗。
