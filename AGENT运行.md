...
r70源码复核时未执行切换；r71已对已验收main完成两次备份/隔离恢复核验并更新家庭实例，原主库和公网入口保留。手机/桌面宽度的恢复副本和实际家长入口流程分别留证。后续增强不阻止已通过当前门槛的独立批次上线；真实手机、微信内访问、打印出纸和持续教学效果仍需分别验收。现有微信采集宿主需要保留进程时，可在短暂停写窗口暂停并恢复原进程及其子进程，不借升级重启客户端或触发重新授权。
