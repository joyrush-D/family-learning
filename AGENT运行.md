...
同一任务总尝试上限已实现。其余成本要求是按明确任务选择轻模型或默认模型、限制每项上下文与输出、给总尝试设限，并记录实际调用用量与升级原因；规则采集、去重与到期判断继续不调用模型。模型调用现先在原有私有SQLite登记，再发请求，记录任务、请求模型、服务返回的模型标识（若提供）、耗时和服务报告的输入／输出／总token；不记录正文或密钥。连接检查也计入，语音转写不在本项内。失败和中断保留，用量缺失记未知；收到结构化答复不代表业务核对或教学质量通过。家长在“家庭设置 → 模型连接 → 近30天模型用量”查看，刷新不会调用模型，记录随原数据库备份恢复，不回填过去的调用。当前任务路由仍未实现，不能将每轮三次当作整日或整项预算，也不用输出上限冒充实际token消耗。
