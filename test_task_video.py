...
class VideoTranscribeTests(TaskVideoTests):
    ...
    def setUp(self):
        ...
        self.reader=self.app.agent_store(read_only=True);self.expected=None  # the store the HTTP entries use: initialize=False over the read-only connection
    ...
    def transcribe(self,body,store=None):
        """Every call also proves that it writes no row or file, launches no tool itself and calls no vision model. A change injected
        while the call is in flight sets self.expected to the data as the injection left it, so the call's own silence is still checked."""
        before=self.snapshot();self.expected=None
        try:return tv.transcribe(self.app,store or self.reader,body)
        finally:self.assertEqual(self.snapshot(),before if self.expected is None else self.expected)
    ...
    def test_the_selected_original_alone_is_decoded_and_sent_once_and_only_text_for_review_returns(self):
        ...
        self.assertEqual([result[k] for k in ('record_id','upload_id','task_id','transcription_fingerprint','state','saved','text','characters','record_limit','speakers_distinguished','audio_assessed','audience')],
                         [ident,video['upload_id'],self.TASK,video['transcription_fingerprint'],'pending_review',False,self.TEXT,len(self.TEXT),4000,False,False,'parent'])
        self.assertNotIn('fits_record',result)
        ...
        self.asr_text='虚'*4000;self.assertEqual((self.transcribe(self.body(ident))['text'],len(self.asr)),('虚'*4000,3))  # exactly the record limit returns whole
        self.asr_text='虚'*4001;error=self.refused(self.body(ident),422,'transcript_too_long',extractions=1,calls=1)  # one over: refused, not cut
        self.assertEqual(str(error),tv.EXPLANATIONS['transcript_too_long']);self.assertNotIn('虚',str(error))
        with self.app.connect() as c:self.assertEqual(tuple(c.execute('SELECT transcript,transcript_state FROM records WHERE id=?',(ident,)).fetchone()),('',''))
        ...
        self.assertEqual((len(self.extracted),len(self.asr),self.rows('record_video_drafts')),(4,4,2))
        self.refused(self.body(ident),422,'transcript_too_long',extractions=1,calls=1)  # a ready draft changes nothing for the request
        self.asr_text=self.TEXT;self.assertEqual(self.transcribe(self.body(ident))['text'],self.TEXT)
    ...
    def test_a_successful_request_and_the_read_run_no_writing_sql_and_the_fingerprint_follows_the_version(self):
        """Traced at sqlite3.connect itself, so every statement of a connection's own setup is seen too: the ordinary initialized
        store's connect runs its schema statements on each connection, and only the read-only factory the HTTP entries use is zero-write."""
        ident=self.linked(self.TASK);body=self.body(ident);connect=sqlite3.connect;before=self.snapshot()
        def traced(store):
            statements=[]
            def opened(*args,**kwargs):
                c=connect(*args,**kwargs);c.set_trace_callback(statements.append);return c
            with mock.patch.object(sqlite3,'connect',opened):result=tv.transcribe(self.app,store,body);read=tv.view(self.app,store,ident)
            self.assertEqual(self.snapshot(),before);self.assertTrue(statements)
            return result,read,[s for s in statements if s.split()[0].upper() in ('INSERT','UPDATE','DELETE','CREATE','DROP','ALTER','REPLACE')]
        result,read,writes=traced(self.app.agent_store(read_only=True));self.assertEqual(writes,[])
        self.assertEqual((result['text'],read['videos'][0]['transcription_fingerprint'],len(self.asr)),(self.TEXT,body['expected_fingerprint'],1))
        result,_,writes=traced(self.agent);self.assertEqual(result['text'],self.TEXT)  # the same trace on the initialized store sees its setup: the caller's contract, not a masked write
        self.assertTrue(any(s.split()[0].upper()=='CREATE' for s in writes),writes[:3])
    ...
    def test_malformed_foreign_unlinked_disabled_stale_unconfigured_and_failed_requests_launch_nothing_and_write_nothing(self):
        ...
        for failure,status,code in ((family_llm.LLMUnavailable('虚构：未配置 '+self.URL),503,'asr_unconfigured'),(family_llm.LLMDraftError('虚构：连接失败 '+self.URL+' key='+self.KEY),502,'asr_failed'),
                                    (family_llm.LLMDraftError('虚构：服务回复 '+self.TEXT),502,'asr_failed'),(RuntimeError('secret '+self.URL+' '+self.KEY),502,'asr_failed'),
                                    (ValueError('音频不能为空'),502,'asr_failed'),(TypeError('虚构：类型错误'),502,'asr_failed')):
            with self.subTest(failure=type(failure).__name__):  # the adapter's own text, which may name the endpoint, key or reply, never leaves
                self.asr_fails=failure;error=self.refused(body,status,code,extractions=1,calls=1);self.assertEqual(str(error),tv.EXPLANATIONS[code])
        self.asr_fails=None
        for returned,code in ((None,'asr_failed'),(b'virtual',"asr_failed"),(123,'asr_failed'),(['虚构'],'asr_failed'),({'text':'虚构'},'asr_failed'),('','asr_empty'),(' \n\t','asr_empty')):
            with self.subTest(returned=repr(returned)):  # a non-text or empty reply is a fixed failure, never a TypeError or a returned blank
                self.asr_text=returned;error=self.refused(body,502,code,extractions=1,calls=1);self.assertEqual(str(error),tv.EXPLANATIONS[code])
        self.asr_text=self.TEXT;self.assertEqual(self.transcribe(body)['text'],self.TEXT)  # nothing above changed the version
    ...
    def test_a_correction_relink_withdrawal_replaced_original_or_changed_asr_configuration_at_each_stage_refuses_or_drops_the_text(self):
        """Each change is injected while the call is in flight through the ordinary write paths, so the call holds no connection or lock
        during extraction or the request; a witness proves the injection happened, and the call is compared against the data as it left it."""
        def replace(ident):
        ...
        for stage in ('extract','asr'):
            for name,change in (('correct',self.correct),('relink',self.relink),('revoke',lambda ident:lambda:self.enable(False)),('replace',replace),('shorten',shorten),
                                ...
                                ('asr_model',lambda ident:lambda:os.environ.__setitem__('FAMILY_ASR_MODEL','other-model')),
