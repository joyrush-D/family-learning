...
let studyChildID='',studyRecordContext=null;
...
document.addEventListener('click',e=>{const b=e.target.closest('[data-source-tasks]');if(!b)return;const name=b.dataset.sourceTasks;if(!data.children.some(c=>c.name===name))return;child=name;page='tasks';taskView='待跟进';render();window.scrollTo(0,0)});
...
function todayReadingHTML(c){
 const items=(data.reading?.tasks||[]).filter(t=>t.child_id===c.id&&(t.state==='待确认'||(['进行中','需补充'].includes(t.state)&&exactTaskDay(t.planned_on)&&t.planned_on<=data.today)));
 return items.map(t=>`<div class="today-reading"><span>${t.state==='待确认'?'等家长看作品':'阅读回看 · '+esc(t.planned_on)}</span><button data-today-reading="${esc(t.id)}" data-today-owner="${esc(c.id)}">${esc(t.book||'阅读约定')} →</button></div>`).join('');
}
function todayChildHTML(c,active){
 ...
 return `<section class="card today-child" id="today-child-${esc(c.id)}" data-today-child="${esc(c.id)}"><header class="today-child-head"><div><h2>${esc(c.name)}</h2><span class="small muted">${esc([c.grade,c.classroom].filter(Boolean).join(' · '))}</span></div><button data-today-capture="${esc(c.id)}">＋ 记学习</button></header>
 <button class="today-study-entry" data-study-child="${esc(c.id)}">放学后：安排与计时 →</button>
 ...
 <div class="today-priorities"><h3 class="today-section-label">今天要处理</h3>${due.map(t=>{const e=linked.get(t.id);return taskHTML(t,{compact:true,when:e?(e.start_time?e.start_time+' · ':'')+(e.status==='tentative'?'暂定 · 先商量':'今天 · '+(calendarKinds[e.category]||'安排')):'今天到期'})}).join('')||'<p class="today-empty">还没收录今天要处理的事项。</p>'}
 ...
 ${todayReadingHTML(c)}${agentChildHTML(c)}
...
function homeHTML(active){
 const gaps=data.sync_error||Object.values(data.sync||{}).some(s=>s?.last_error||s?.collection_status==='awaiting_login');
 return `<section class="today-dashboard"><header class="today-heading"><h1>今天要做什么</h1><button data-page="calendar">日历与课表 →</button></header>${gaps?'<p class="today-source-gap">学校信息尚有读取缺口 <button data-page="sources">查看来源</button></p>':''}${data.today_calendar?.source_error?`<p class="error" role="status">${esc(data.today_calendar.source_error)}</p>`:''}<div class="today-jump" aria-label="跳到孩子的今日事项">${data.children.map(c=>`<a href="#today-child-${esc(c.id)}">${esc(c.name)}</a>`).join('')}</div><div class="today-children">${data.children.map(c=>todayChildHTML(c,active)).join('')}</div>
 ${agentStatusHTML(false)}<div class="today-shortcuts"><button data-page="tasks">全部待办 · ${active.length}</button><button data-page="reading">阅读与作品</button><button data-page="print">打印资料</button><button data-page="care">陪伴建议</button></div>${data.care?.error?`<p class="error" role="status">${esc(data.care.error)}</p>`:''}
 <details class="today-record-guide"><summary>怎么留下学习进展？</summary><p>点孩子旁边的“记学习”，说一句今天学了什么、哪里卡住；也可以拍作业或试卷。识别结果先核对，再保存。</p><p>孩子愿意时，再约一次简短聊天，了解他喜欢什么、希望得到什么帮助。</p></details></section>`;
}
...
function render(){if(!data)return;const savedFocus=captureContentFocus();if(page!=='calendar')window.FamilyCalendar?.leave();window.FamilyStudy?.leave();window.FamilySettings?.leave();window.FamilyWorld?.destroy();let html='';const ts=selected(data.tasks),rs=selected(data.records);const active=ts.filter(t=>!taskClosed(t));
if(page==='home'){
html=homeHTML(active);
 }else if(page==='tasks'){
 const views={'待跟进':active.filter(t=>taskFocus(t).mode==='next'||taskReviewDue(t)),'等待中':active.filter(t=>taskFocus(t).mode==='waiting'),'以后再说':active.filter(t=>taskFocus(t).mode==='later'),'已完成':ts.filter(t=>status(t)==='已完成'),'已搁置':ts.filter(taskDismissed),'全部':ts};if(!views[taskView])taskView='待跟进';
 html=`<h1>家庭行动清单</h1><div class="task-list-top">${filters()}<button data-new-task="yes">＋ 收集一件事</button></div><div class="tasktabs" aria-label="筛选事项">${Object.entries(views).map(([name,items])=>`<button data-view="${name}" aria-pressed="${taskView===name}">${name==='待跟进'?'下一步':name} <span>${items.length}</span></button>`).join('')}</div>${taskView==='等待中'?'<p class="small muted">记住在等谁、等什么，到约定日期再跟进。</p>':taskView==='以后再说'?'<p class="small muted">回看日期不会改变原截止要求；到期后也会重新出现在“下一步”。</p>':''}<section class="card checklist">${views[taskView].map(t=>taskHTML(t)).join('')||empty('这里暂时没有事项。')}</section>`;
...
html=`<h1>学习进展</h1><button data-page="reading">打开阅读营地 →</button><p class="muted">把尝试、订正和复测放在一起，看看做到了什么、还需要什么帮助。</p>${filters()}${window.FamilyGuided?.html()||''}${learningJourneysHTML()}<section class="card"><h2>成绩记录</h2><select id="subjectFilter" aria-label="筛选成绩科目"><option value="">全部科目</option>${subjects.map(s=>`<option ${s===subject?'selected':''}>${esc(s)}</option>`).join('')}</select>${scores.length?`<p class="note">柱高为得分比例；不同考试范围、难度与满分不能直接判断进退。</p><div class="chart">${scores.slice(0,8).reverse().map(r=>`<div class="bar" style="height:${Math.max(2,r.score/r.total*100)}%"><span>${esc(r.score)}/${esc(r.total)}</span><small>${esc(r.day.slice(5))}<br>${esc(r.subject)} · ${esc(r.child)}</small></div>`).join('')}</div>${scores.map(recordHTML).join('')}`:empty('尚无成绩数据，不推算分数或排名。可以从最近一张试卷开始记录。')}</section><section class="card"><h2>学习过程</h2>${rs.filter(r=>r.category==='学习进展').map(recordHTML).join('')||empty('尚无个人学习进展记录；老师布置了作业，不代表孩子已经完成。')}</section><section class="card"><h2>已有学习观察与背景</h2><details><summary>查看成长档案</summary><pre>${esc(data.growth)}</pre></details></section>`;
...
}else if(page==='study'){html='<div class="toolbar"><button data-page="home">返回今天</button><button data-page="calendar">晚饭、运动与其它安排</button></div><div id="studyRoot"></div>';
...
async function submit(e,path,dialog,error){
 e.preventDefault();const f=e.target,b=f.querySelector('[type=submit]'),care=f===$('#recordForm')&&careFeedbackContext;
 ...
 try{
  ...
  try{await load()}catch{toast('已保存，但列表暂时无法刷新。请稍后点“刷新记录”核对。')}
...
function invalidateDraft(){draft=null;$('#draftResult').innerHTML='';$('#draftStatus').textContent=data?.llm?.configured?'资料或文字已变化，可重新整理草稿。':'识别服务未配置，仍可保存原件和手动记录。'}
...
let drafting=false,draft=null;
$('#draftButton').onclick=async()=>{if(drafting||uploading||recorder||micPending||readingBusy)return;const f=$('#recordForm'),text=[f.elements.title.value,f.elements.note.value].filter(Boolean).join('\n');drafting=true;f.elements.title.readOnly=true;f.elements.note.readOnly=true;captureLock(true);$('#draftButton').disabled=true;$('#recordAudio').disabled=true;$('#draftStatus').textContent='正在整理，请稍等。原件已经保存。';$('#draftResult').innerHTML='';draft=null;const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),90000);try{const r=await apiFetch('/api/draft',{method:'POST',signal:controller.signal,headers:{'Content-Type':'application/json','X-Family-Token':data.token},body:JSON.stringify({text,attachments:pendingIDs.filter(id=>['image/jpeg','image/png','image/webp'].includes((data.uploads||[]).find(a=>a.id===id)?.mime))})});const result=await r.json();if(!r.ok)throw Error(result.error);draft=result.draft;$('#draftResult').innerHTML=`<div class="note"><strong>请核对这份草稿</strong><p>${esc(draft.title)}</p><p>${esc(draft.subject)}${draft.score!==null?' · '+esc(draft.score)+' / '+esc(draft.total??'满分待核对'):''}</p><p class="source">${esc(draft.note)}</p>${draft.uncertainties.length?`<p>待核对：${draft.uncertainties.map(esc).join('；')}</p>`:''}<button type="button" id="applyDraft">填入表单，继续核对</button></div>`;$('#draftStatus').textContent='草稿尚未写入成长记录。请核对，再保存。';$('#applyDraft').onclick=()=>{f.elements.title.value=draft.title;f.elements.subject.value=draft.subject;f.elements.note.value=(draft.note+(draft.uncertainties.length?'\n待核对：'+draft.uncertainties.join('；'):'')).slice(0,4000);f.elements.score.value=draft.score??'';f.elements.total.value=draft.total??'';f.elements.category.value=draft.score!==null&&draft.total!==null?'成绩':'学习进展';$('#scoreFields').classList.toggle('hide',f.elements.category.value!=='成绩');$('#draftStatus').textContent='已填入表单，可修改任何字段；点击保存记录才会入档。';};}catch(err){$('#draftStatus').textContent=(err.name==='AbortError'?'整理超时，请稍后重试':err.message)+'。原件和当前填写内容仍保留。';}finally{clearTimeout(timer);drafting=false;f.elements.title.readOnly=false;f.elements.note.readOnly=false;captureLock(false);$('#draftButton').disabled=!data.llm?.configured;}};
...
document.addEventListener('click',async e=>{
  ...
  if(!['record','task','care','reading','reading_balance','reading_redemption','calendar','timetable'].includes(c.kind)||askState.openingSource)return;
  ...
  if(target){target.scrollIntoView({block:'center'});target.focus({preventScroll:true})}else toast('原记录可能已更新或建议已过期，请刷新后重新查询。');
});
...
document.addEventListener('click',e=>{
 ...
 if(b.dataset.todayReading){child=data.children.find(c=>c.id===b.dataset.todayOwner)?.name||'';page='reading';readingView='在读';render();const target=[...document.querySelectorAll('[data-reading-target]')].find(t=>t.dataset.readingTarget===b.dataset.todayReading);target?.scrollIntoView({block:'center'});target?.focus({preventScroll:true});}
});

function agentTime(value){return value?new Date(value).toLocaleString('zh-CN',{month:'numeric',day:'numeric',hour:'2-digit',minute:'2-digit',hour12:false}):'尚无成功记录'}
...
function agentItemHTML(item){
 const c=data.children.find(c=>c.id===item.child_id),care=item.care_id&&(data.care?.items||[]).some(c=>c.id===item.care_id),record=item.record_id&&data.records.some(r=>r.id===item.record_id);
 return `<article class="agent-item" data-agent-item="${esc(item.id)}"><span class="small muted">${esc(c?.name||'归属待核对')} · ${item.kind==='school'?'学校新信息':item.kind==='review'?'到期回看':'学习跟进'}</span><h3>${esc(item.title)}</h3><p>${esc(item.body)}</p>${item.due?`<p class="small">日期：${esc(item.due)}</p>`:''}<details><summary>查看依据</summary>${(item.evidence||[]).map(e=>`<blockquote>${esc(e.text||e.quote)}</blockquote><p class="small muted">${esc(e.ref)}</p>`).join('')}</details><div class="toolbar">${item.state==='accepted'?`<button data-task="${esc(item.task_id)}">跟进待办</button>`:item.kind==='school'?`<button data-agent-accept="${esc(item.id)}">核对并加入待办</button>`:care?`<button data-care="${esc(item.care_id)}">记录选择或反馈</button>`:record?`<button data-followup="${esc(item.record_id)}">补充实际进展</button>`:`<button data-today-capture="${esc(item.child_id)}">记一次尝试</button>`}${item.state==='pending'?`<button data-agent-dismiss="${esc(item.id)}">${item.kind==='school'?'忽略这条':'已看过'}</button>`:''}</div></article>`;
}
function agentChildHTML(c){const items=agentPending(c.id);return items.length?`<div class="agent-child"><h3 class="today-section-label">助手提醒 · ${items.length}</h3>${items.slice(0,2).map(agentItemHTML).join('')}${items.length>2?'<button data-page="agent">查看其余提醒 →</button>':''}</div>`:''}
function agentStatusHTML(full){
 ...
 const fault=a.last_error||stale||a.sources?.some(s=>s.enabled&&(s.error||!s.last_success||Date.now()-Date.parse(s.last_success)>15*60*1000));
 return `<section class="agent-status" aria-label="成长助手运行状态"><div><strong>成长助手</strong><span class="small">${fault?'有资料等待处理':state} · ${a.pending_count||0} 条待看</span></div>${full?'':'<button data-page="agent">查看提醒与状态 →</button>'}${full?`<p class="small">上次整理：${agentTime(a.last_run)}${stale?' · 已超过10分钟，请检查后台服务':''}</p>${a.last_error?`<p role="status" class="error">${esc(a.last_error)}</p>`:''}${a.failed_jobs?`<p class="error">${a.failed_jobs} 批资料仍未整理成功 <button data-agent-retry>重新尝试</button></p>`:''}<details><summary>消息来源与读取情况</summary>${(a.sources||[]).map(s=>`<div class="agent-source"><strong>${esc(s.name)}</strong><p class="small">最近成功读取：${agentTime(s.last_success)}<br>最近尝试：${agentTime(s.last_attempt)}</p>${s.unread_count?`<p class="small">${s.unread_count} 条包含尚未读取的附件、图片或截断内容，请到原群核对。</p>`:''}${s.error?`<p class="error">${esc(s.error)}</p>`:!s.last_success?'<p class="small">尚未读取，不能据此判断没有新消息。</p>':Date.now()-Date.parse(s.last_success)>15*60*1000?'<p class="small">读取已超过15分钟，请检查采集电脑和连接。</p>':''}</div>`).join('')||'<p>尚未配置消息来源，仍可处理已保存的学习记录。</p>'}</details><p class="small muted">系统在后台整理；原件未读及历史未覆盖的内容仍需核对。此处提醒不代表已向微信或手机发送通知。</p>`:''}</section>`;
}
...
async function agentAction(obj){const r=await apiFetch('/api/agent/action',{method:'POST',headers:{'Content-Type':'application/json','X-Family-Token':data.token},body:JSON.stringify(obj)});const result=await r.json();if(!r.ok)throw Error(result.error||'处理失败，请重试');return result}
function openAgentTask(id){
 const item=agentPending().find(x=>x.id===id);if(!item)return;
 let dialog=$('#agentDialog');if(!dialog){dialog=document.createElement('dialog');dialog.id='agentDialog';dialog.innerHTML='<form id="agentForm"><h2>核对学校待办</h2><input type="hidden" name="id"><label>要做什么<input name="title" required maxlength="200"></label><label>哪一天<input name="due" type="date"></label><label>准备或要求<textarea name="body" rows="4" maxlength="4000"></textarea></label><p class="small muted">日期不明确可以留空，加入后仍是待跟进。</p><p id="agentError" class="error" role="alert"></p><div class="toolbar"><button type="button" data-close="agentDialog">取消</button><button type="submit" class="primary">加入待办</button></div></form>';document.body.append(dialog);$('#agentForm').onsubmit=async e=>{e.preventDefault();const f=e.currentTarget,b=f.querySelector('[type="submit"]');if(b.disabled)return;b.disabled=true;$('#agentError').textContent='';try{await agentAction({action:'accept',...Object.fromEntries(new FormData(f))});dialog.close();await load();toast('已加入待办，可在清单里跟进')}catch(err){$('#agentError').textContent=err.message}finally{b.disabled=false}}}
 const f=$('#agentForm');f.reset();f.elements.id.value=id;f.elements.title.value=item.title;f.elements.due.value=exactTaskDay(item.due);f.elements.body.value=item.body;$('#agentError').textContent='';dialog.showModal();
}
document.addEventListener('click',async e=>{const b=e.target.closest('button');if(!b||!data)return;if(b.dataset.agentAccept){openAgentTask(b.dataset.agentAccept);return}if(!b.hasAttribute('data-agent-dismiss')&&!b.hasAttribute('data-agent-retry')&&!b.hasAttribute('data-agent-refresh'))return;if(b.disabled)return;b.disabled=true;try{if(b.hasAttribute('data-agent-dismiss'))await agentAction({id:b.dataset.agentDismiss,action:'dismiss'});if(b.hasAttribute('data-agent-retry'))await agentAction({action:'retry'});await load();if(b.hasAttribute('data-agent-retry'))toast('已安排重试，后台下一轮会处理')}catch(err){toast(err.message)}finally{b.disabled=false}});
// One lightweight saved-state read per minute; never collect or call a model on page visits.
setInterval(async()=>{if(!data?.agent?.enabled||document.hidden||document.querySelector('dialog[open]')||!['home','agent'].includes(page)||document.activeElement?.matches('input,textarea,select'))return;try{const r=await apiFetch('/api/agent',{signal:AbortSignal.timeout(8000)});if(!r.ok)return;const next=await r.json();if(document.querySelector('dialog[open]'))return;if(JSON.stringify(next)!==JSON.stringify(data.agent)){data.agent=next;render()}}catch{}},60000);

function mountStudy(){
 const root=$('#studyRoot');if(!window.FamilyStudy){root.innerHTML=empty('放学后安排暂未加载，请刷新后重试。');return}
 window.FamilyStudy.mount({root,child_id:studyChildID||data.children[0]?.id,day:data.today,children:data.children,apiFetch,token:data.token,
  ...
  onRecord:id=>{const record=data.records.find(r=>r.id===Number(id));if(!record){toast('记录已保存，请刷新后查看。');return}child=record.child;page='learning';render();const target=document.querySelector('[data-query-target="record:'+Number(id)+'"]');target?.scrollIntoView({block:'center'});target?.focus({preventScroll:true})}
 });
}
document.addEventListener('click',e=>{const b=e.target.closest('[data-study-child]');if(!b||!data)return;studyChildID=b.dataset.studyChild;page='study';render();window.scrollTo(0,0)});
...
function openTaskFocus(id){
 ...
 for(const key of ['mode','next_action','waiting_for','review_on'])f.elements[key].value=focus[key]||'';
