...
}else if(page==='more'){html=`<h1>更多</h1><section class="card more-links">${[['print','打印作业'],['sources','消息来源与附件'],['goals','学习目标与每日反馈'],['learning','学习任务与进展'],['teachers','老师档案'],['growth','成长记录'],['reading','阅读与作品'],['care','陪伴建议'],['agent','助手跟进'],['settings','家庭设置']].map(([key,label])=>`<button data-page="${key}">${label} →</button>`).join('')}</section>${parentLogoutAvailable?'<button type="button" data-parent-logout>退出这台设备</button>':''}`;
...
function agentItemHTML(item){
 ...
 if(item.goal_id)actions=`<button data-goal-id="${esc(item.goal_id)}" data-goal-child="${esc(item.child_id)}">查看目标 / 反馈 / 审核</button>`;
 if(item.state==='pending'&&!item.goal_id)actions+=`<button data-agent-dismiss="${esc(item.id)}">${item.kind==='school'?'忽略这条':isPlannedCare?'暂不考虑':'已看过'}</button>`;
