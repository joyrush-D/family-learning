...
let studyChildID='',studyRecordContext=null,schoolRecordContext=null,schoolRecordOpening=false;
...
function agentItemHTML(item){
 ...
 return `<article class="agent-item" data-agent-item="${esc(item.id)}"><span class="small muted">${esc(c?.name||'归属待核对')} · ${item.kind==='school'?'学校信息':item.kind==='review'?'到期回看':'学习跟进'}</span><h3>${esc(item.title)}</h3><p>${esc(item.body)}</p>${item.due?`<p class="small">日期：${esc(item.due)}</p>`:''}<details><summary>查看依据</summary>${(item.evidence||[]).map(e=>`<blockquote>${esc(e.text||e.quote)}</blockquote><p class="small muted">${esc(e.ref)}</p>`).join('')}</details><div class="toolbar">${item.state==='accepted'?`<button data-task="${esc(item.task_id)}">跟进待办</button>`:item.kind==='school'?`<button data-agent-accept="${esc(item.id)}">核对并加入待办</button>`:care?`<button data-care="${esc(item.care_id)}">记录选择或反馈</button>`:record?`<button data-followup="${esc(item.record_id)}">补充实际进展</button>`:`<button data-today-capture="${esc(item.child_id)}">记一次尝试</button>`}${item.kind==='school'?`<button data-school-record-agent="${esc(item.id)}">留作学习记录</button>`:''}${item.state==='pending'?`<button data-agent-dismiss="${esc(item.id)}">${item.kind==='school'?'忽略这条':'已看过'}</button>`:''}</div></article>`;
}
