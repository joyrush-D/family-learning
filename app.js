...
let data, page='calendar', child='', subject='', taskView='待整理', busy=false, pendingTask=null;
...
async function postTask(obj){
 ...
 if(result.task?.id===obj.id){
  ...
  data.tasks=data.tasks.map(t=>t.id===obj.id?result.task:t);if(data.today_calendar?.inbox){const row=data.today_calendar.inbox.find(x=>x.task_id===obj.id);if(row){row.status=status(result.task);row.closed=taskClosed(result.task);row.closed_on=(result.task.update?.updated||'').slice(0,10)}}window.FamilyCalendar?.invalidate();
