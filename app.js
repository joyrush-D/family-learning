...
function taskHTML(t,options={}){
 const compact=options.compact===true,done=status(t)==='已完成',dismissed=taskDismissed(t),pending=pendingTask?.id===t.id,open=!taskClosed(t);
 const primary=open&&t.focus?.box!=='wish'&&t.agenda?.category==='homework'?`<button class="primary" data-study-task-add="${esc(t.id)}">作业计时</button>`:open&&t.title.includes('打印')?'<button class="primary" data-page="print">准备打印</button>':'';
...
function agentItemHTML(item,options={}){
 ...
 if(options.compact&&item.kind==='school'){
  const excerpt=displayTitle.replace(/^待核对[：:]\s*/,'').split('\n')[0],short=excerpt.length>64?excerpt.slice(0,64)+'…':excerpt,goal=displayBody.startsWith('请核对这条通知是否适用')?'':displayBody;
  return `<article class="agent-item school-review" data-agent-item="${esc(item.id)}"><div class="task-meta"><span>${esc(c?.name||'归属待核对')}</span><span class="review-badge">待核对通知</span></div><h3>${esc(short)}</h3>${agendaDateHTML(options.agenda||{})}${goal?`<div class="task-goal"><p>${esc(goal)}</p></div>`:''}<div class="tasktools"><button class="primary" data-agent-accept="${esc(item.id)}">核对要求</button><button data-agent-dismiss="${esc(item.id)}">忽略这条</button></div><details class="task-reference task-more"><summary>原通知与其他操作</summary>${schoolOriginalButtons((item.evidence||[]).map(e=>e.ref),item.child_id)}<p>${esc(displayTitle)}</p>${(item.evidence||[]).map(e=>`<blockquote>${esc(e.text||e.quote)}</blockquote><p class="small muted">${esc(e.ref)}</p>`).join('')}${item.plan?.school_task?.advice?`<p>操作建议：${esc(item.plan.school_task.advice)}</p>`:''}<div class="tasktools"><button data-school-record-agent="${esc(item.id)}">留作学习记录</button>${item.goal_id?`<button data-goal-id="${esc(item.goal_id)}" data-goal-child="${esc(item.child_id)}">查看学习目标</button>`:''}</div></details></article>`;
 }
