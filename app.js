...
}else if(page==='more'){html=`<h1>更多</h1><section class="card more-links">${[['print','打印作业'],['sources','消息来源与附件'],['learning','学习任务与进展'],['teachers','老师档案'],['growth','成长记录'],['reading','阅读与作品'],['care','陪伴建议'],['agent','助手跟进'],['settings','家庭设置']].map(([key,label])=>`<button data-page="${key}">${label} →</button>`).join('')}</section>${parentLogoutAvailable?'<button type="button" data-parent-logout>退出这台设备</button>':''}`;
