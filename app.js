...
function todayChildHTML(c,active){
 ...
 return `<section class="card today-child" id="today-child-${esc(c.id)}" data-today-child="${esc(c.id)}"><header class="today-child-head"><div><h2>${esc(c.name)}</h2><span class="small muted">${esc([c.grade,c.classroom].filter(Boolean).join(' · '))}</span></div><button data-today-capture="${esc(c.id)}">＋ 拍照 / 补充</button></header>
 <button class="today-study-entry" data-study-child="${esc(c.id)}">今日作业 · 安排与计时 →</button>
...
function mountStudy(){
 ...
 window.FamilyStudy.mount({root,child_id:studyChildID||data.children[0]?.id,task_id:studyTaskID,onTaskSelected:()=>studyTaskID='',day:data.today,children:data.children,apiFetch,token:data.token,
