...
function prepareTaskCapture(task,record=null){
 ...
 $('#taskCaptureHome').append(sharedCapture);captureHint.textContent='每份最多20MB。录音或照片随这项任务保存；语音可转写核对，图片本次只保留原件。';$('#draftButton').classList.add('hide');$('#draftResult').innerHTML='';$('#draftStatus').textContent='';$('#uploadStatus').textContent='';
...
function drawTaskFeedback(task){
 const records=data.records.filter(r=>r.source==='事项:'+task.id&&r.child===task.child);
 $('#taskFeedbackHistory').innerHTML=records.length?'<h3>这项任务的反馈</h3>'+records.map(r=>`<article class="note"><p>${esc(r.day)} · ${esc(r.note||(r.transcript?'已保存语音转写':'已保存原件，内容待核对'))}</p>${r.transcript?`<p class="source">转写（${esc(r.transcript_state)}）：${esc(r.transcript)}</p>`:''}${(r.attachments||[]).map(id=>data.uploads.find(a=>a.id===id)).filter(Boolean).map(uploadHTML).join('')}<button type="button" data-task-feedback-edit="${r.id}">更正这条反馈</button></article>`).join(''):'';
}
...
$('#saveTaskFeedback').onclick=async()=>{
 ...
 const body=taskFeedbackPending;f.dataset.saving='yes';lockTaskFeedback(true);$('#saveTaskFeedback').textContent='正在保存反馈…';$('#taskError').textContent='';
...
function captureLock(locked){if(taskFeedbackContext){$('#saveTaskFeedback').disabled=locked;$('#taskForm [type=submit]').disabled=locked;}$('#transcribeButton').disabled=locked||!data?.asr?.configured||selectedAudio().length!==1;$('#draftButton').disabled=locked||!data?.llm?.configured;$('#fileInput').disabled=locked;$('#cameraInput').disabled=locked;$('#recordForm [type=submit]').disabled=locked;$('#recordAudio').disabled=uploading;}
...
window.addEventListener('beforeunload',e=>{if(uploading||recorder||micPending||drafting||failedFiles.length||careFeedbackPending||taskFeedbackPending||timetableBusy||timetablePending){e.preventDefault();e.returnValue='';}});
