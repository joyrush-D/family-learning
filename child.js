...
function showArea(area, focus=false) {
  ...
  if (!state?.study_enabled) { window.FamilyHomework?.clear(); study.snapshot=null; $('study-items').replaceChildren(); $('study-summary').replaceChildren(); $('study-editor').replaceChildren(); }
...
function wireStudy() {
  $('study-report-material').onclick=()=>{if(!state?.study_enabled||study.busy||study.pending||operation||recorder||micPending)return;const day=study.day;window.FamilyHomework.open({key:'child:'+identityGeneration+':'+day,child:true,day,child_name:state.child?.name||'',request:(action,body)=>api('study/'+action,{...body,day},{timeout:120000}),upload:file=>api('upload',file,{raw:true,timeout:120000,headers:{'Content-Type':'application/octet-stream','X-File-Name':encodeURIComponent(file.name)}}),transcribe:id=>api('transcribe',{attachment:id},{timeout:90000}),fileURL:id=>new URL('upload/'+id,base).href,onSaved:()=>readStudy()})};
...
function renderGuide(focus=false){const s=guideCurrent();setGuideMode(true);$('child-study').hidden=true;$('journey').hidden=true;$('camp-tabs').hidden=true;$('guided-home').hidden=true;$('workshop').hidden=false;$('saved-work').hidden=true;const d=guideDraft();
 ...
 $('work-title').textContent=s.title;$('guided-problem').innerHTML=`${s.learning_goal?`<section class="guided-goal" aria-label="这次学习目标"><strong>这次一起练</strong><p class="work-copy">${esc(s.learning_goal.goal)}</p><p class="small work-copy">试着做到：${esc(s.learning_goal.success_criteria)}</p></section>`:''}<p class="work-copy">${esc(s.question_text)}</p><div id="guided-question-originals"></div>${s.material_gaps.length?`<p class="small">${s.material_gaps.map(esc).join('；')}</p>`:''}`;renderAttachments(s.question_attachments.map(a=>({...a,status:'saved'})),'guided-question-originals',false);
 const attempts=s.events.filter(e=>e.kind==='attempt'),lastAttemptID=attempts.at(-1)?.id||0,hint=[...s.events].reverse().find(e=>e.kind==='hint'&&e.id>lastAttemptID);
...
function lockGuide(){const s=guideCurrent(),d=guideDraft(),locked=operation||!!recorder||micPending||guided.busy||guided.hintBusy||!!guided.pending||guided.conflict;
 ...
 $('rest').disabled=locked||!s?.allowed_actions.includes('attempt');$('back').disabled=operation||!!recorder||micPending||guided.busy||!!guided.pending;$('refresh').disabled=operation||!!recorder||micPending||guided.busy;$('logout').disabled=operation||!!recorder||micPending||guided.busy;
