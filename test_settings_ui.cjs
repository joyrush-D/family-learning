...
(async()=>{
 ...
 try{
  ...
  for(const width of [360,1440]){
   ...
   // The top bar is hidden on 今天; parents reach 家庭设置 from 更多 on the current layout.
   const openSettings=async()=>{await p.locator('nav [data-page="more"]').click();await p.locator('.more-links [data-page="settings"]').click();await p.locator('.settings-view').waitFor()};
   try{
    ...
    await p.locator('.settings-start').click();await eventually(async()=>await p.locator('.settings-view').count()===0,'leave settings');await openSettings();await eventually(async()=>await p.locator('.settings-source').count()===3,'return to saved settings');await fit(p);await proof(p,'settings-saved-'+width);
    await p.reload();await openSettings();assert.equal(await form('model').locator('[name="api_key"]').inputValue(),'');assert.equal((await read()).children.length,2);assert.deepEqual(errors,[]);
