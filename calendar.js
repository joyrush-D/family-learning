...
function taskGroupsHTML(items,homeworkLabel='课内作业'){
 return [['homework',homeworkLabel],['todo','待办事项']].map(([kind,label])=>{const rows=items.filter(x=>(x.agenda.category==='homework'?'homework':'todo')===kind);return `<section class="card agenda-group"><h2>${label} · ${rows.length}</h2>${rows.map(agendaItemHTML).join('')||'<p class="small muted">没有已收录的事项</p>'}</section>`}).join('');
}
